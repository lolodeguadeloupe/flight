-- Migration script corrigé pour créer les tables de base pour l'application Flight Compensation
...
-- 4. Table des réclamations (claims) - utilise BIGSERIAL pour l'ID principal
...
-- 5. Table des documents - utilise BIGINT pour claim_id
...
DROP TRIGGER IF EXISTS handle_flights_updated_at ON public.flights;
...
-- 11. Permissions publiques pour les aéroports (lecture seulement)
ALTER TABLE public.airports ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Anyone can view airports" ON public.airports FOR SELECT USING (true);

-- Fin du script de migration corrigé