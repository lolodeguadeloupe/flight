...
class FlightCompensationApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return MaterialApp(
      ...
      home: ref.watch(appInitializationProvider).when(
        data: (initialized) {
          if (initialized) {
            return const HomePage();
          } else {
            return const AppInitializationErrorPage();
          }
        },
        loading: () => const AppLoadingPage(),
        error: (error, stack) => AppInitializationErrorPage(error: error),
      ),
    );
  }
}
...
// ============================================================================
// App Initialization UI Components
// ============================================================================
...
class AppInitializationErrorPage extends StatelessWidget {
  ...
  const AppInitializationErrorPage({super.key, this.error});
...
class _ClaimFormPageState extends ConsumerState<ClaimFormPage> {
  ...
  Future<void> _submitClaim() async {
    ...
    try {
      ...
      // Get current user
      final currentUserResult = await ref.read(currentUserProvider.future);
      if (currentUserResult == null) {
        throw Exception('User not authenticated');
      }

      // Create a Claim entity (this would need proper implementation based on your domain models)
      // For now, this is a placeholder - you'll need to create the proper Claim entity
      // final claim = Claim(...);
      
      // Submit claim using new provider architecture
      // final result = await ref.read(submitClaimProvider(claim).future);
      
      // Temporary fallback to old implementation
      final result = {'id': DateTime.now().millisecondsSinceEpoch.toString()};
